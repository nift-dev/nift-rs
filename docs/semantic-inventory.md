...
|---|---|---|
| `build`/`build --all` CLI, incremental build machinery | contract | unsupported; any future CLI summary calls tracked outputs "files" (for example `3 files built successfully`) and status calls them "tracked files" |
