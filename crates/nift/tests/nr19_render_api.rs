...
fn render_path_and_text_are_typed() {
    ...
    write(&root.join(".nift/config.json"),
    ...
    write(&root.join(".nift/tracked.json"),
          r#"{"tracked":[{"name":"about","title":"About","template":"templates/template.html"}]}"#);
    write(&root.join("templates/template.html"), "<main>@content</main>");
    ...
    assert!(engine.render_path(root.join("nope.html"), &ctx).is_err(),
    ...
    let via_text = engine.render_text("<p>literal</p>", &ctx).expect("render_text");
    assert_eq!(via_text.output, "<p>literal</p>");
    let names_a_file = root.join("content/about.html").to_string_lossy().to_string();
    let literal = engine.render_text(&names_a_file, &ctx).expect("render_text literal");
    ...
    let tt = engine
        .render(&Source::text("<p>hi</p>"), &Source::text("<main>@content</main>"), &ctx)
